...
bool BCI2000Reader::parse_meta(std::string &meta)
{
    ...
    else
    {
        ...
        {
            std::cerr << " *** error: cannot parse meta information header" << std::endl;
            return false;
        }
    }
    return true;
...
#else
    std::cerr << "read_samples from int16 or int32 is not implemented yet on bigendian machines" << std::endl;
    exit(EXIT_FAILURE);
#endif
