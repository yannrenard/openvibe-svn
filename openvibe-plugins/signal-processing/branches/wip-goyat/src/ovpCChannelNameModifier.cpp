...
void CChannelNameModifier::setChannelCount(const uint32 ui32ChannelCount)
{
	//gets the channel count
	m_pSignalDescription->m_ui32ChannelCount = ui32ChannelCount;
}

void CChannelNameModifier::setChannelName(const uint32 ui32ChannelIndex, const char* sChannelName)
{
	//adds this name to the list
	m_vChannelNames.push_back(sChannelName);
}

void CChannelNameModifier::setSampleCountPerBuffer(const uint32 ui32SampleCountPerBuffer)
{
	...
	if(m_bSelectionbyIndex)
	{
		...
		for(unsigned int i=0 ; i<m_vSelectedChannelIndexes.size() ; i++)
		{
			...
			if(l_ui32CurrentChannel>=0 && l_ui32CurrentChannel<m_pSignalDescription->m_ui32ChannelCount)
			{
				...
				//char buffer[40]="0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ";
				string str=m_vChannelNewNames.size()>i ? m_vChannelNewNames[i] : "UNKNOWN";//"TEST"
				//str.push_back(buffer[l_ui32CurrentChannel]);
				m_pSignalDescription->m_oChannelName.push_back(str);
			}
		}
	}
	...
	else
	{
		...
		for(uint32 i=0 ; i<m_pSignalDescription->m_ui32ChannelCount ; i++)
		{
			...
			//m_pSignalOutputWriterHelper->setChannelName(i, "TestBis");
		}
...
void CChannelNameModifier::setSamplingRate(const uint32 ui32SamplingFrequency)
{
	m_pSignalDescription->m_ui32SamplingRate = ui32SamplingFrequency;
}

void CChannelNameModifier::setSampleBuffer(const float64* pBuffer)
{
...
void CChannelNameModifier::writeSignalOutput(const void* pBuffer, const EBML::uint64 ui64BufferSize)
{
	appendOutputChunkData<0>(pBuffer, ui64BufferSize);
}

CChannelNameModifier::CChannelNameModifier(void)
	: m_pReader(NULL),
	...
	m_oSignalOutputWriterCallbackProxy(
		*this,
		&CChannelNameModifier::writeSignalOutput),
...
void CChannelNameModifier::release(void)
{
}

boolean CChannelNameModifier::initialize()
{
	//reads the plugin settings
	CString l_sSettings;
	getBoxAlgorithmContext()->getStaticBoxContext()->getSettingValue(1, l_sSettings);
	m_bSelectionbyIndex =(l_sSettings == CString("true")? true : false);
	...
	//reads the plugin settings2
	CString l_sSettings2;
	getBoxAlgorithmContext()->getStaticBoxContext()->getSettingValue(2, l_sSettings2);
	istringstream l_sChannelList2(static_cast<const char *>(l_sSettings2));

		//parses the configuration string
	string l_sCurrentChannelString;
	while(!l_sChannelList2.eof())
	{
		getline(l_sChannelList2, l_sCurrentChannelString, OV_Value_EnumeratedStringSeparator);

		//We don't want empty strings (causes problems when comparing with input stream's channel's labels)
		if(l_sCurrentChannelString.length() != 0)
		{
			m_vChannelNewNames.push_back(l_sCurrentChannelString);
		}
	}


	//initialises the signal description structure
	m_pSignalDescription = new CSignalDescription2();
...
boolean CChannelNameModifier::uninitialize()
{
	if(m_pMatrixBuffer)
	{
...
boolean CChannelNameModifier::processInput( uint32 ui32InputIndex)
{
	getBoxAlgorithmContext()->markAlgorithmAsReadyToProcess();
	return true;
}

boolean CChannelNameModifier::process()
{
