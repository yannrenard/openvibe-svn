...
boolean CAlgorithmSingleTrialPhaseLockingValue::initialize(void)
{
    ...
    for(i=0; i<m_pChannelPairs->getDimensionSize(1); i=i+2)
    {
    	if(m_pChannelPairs->getBuffer()[i] < m_pInputMatrix1->getDimensionSize(0))
    	{
    		System::Memory::copy(m_pChannelToCompare->getBuffer()+i*m_ui32SamplesPerChannel1, m_pInputMatrix1->getBuffer()+m_pChannelPairs->getBuffer()[i]*m_ui32SamplesPerChannel1,
    							m_ui32SamplesPerChannel1*sizeof(float64));
    	}
    }

    for(i=1; i<m_pChannelPairs->getDimensionSize(1); i=i+2)
        {
        	if(m_pChannelPairs->getBuffer()[i] < m_pInputMatrix2->getDimensionSize(0))
        	{
        		System::Memory::copy(m_pChannelToCompare->getBuffer()+i*m_ui32SamplesPerChannel2, m_pInputMatrix2->getBuffer()+m_pChannelPairs->getBuffer()[i]*m_ui32SamplesPerChannel2,
...
boolean CAlgorithmSingleTrialPhaseLockingValue::process(void)
{
	    ...
	    complex <double> iComplex(0.0,1.0);
	    ...
	    if(this->isInputTriggerActive(OVTK_Algorithm_Connectivity_InputTriggerId_Process))
	   	{
	    	...
	    	{
		    	l_vecXdChannelToCompare1 = RowVectorXd::Zeros(m_ui32SamplesPerChannel1);
		    	l_vecXdChannelToCompare2 = RowVectorXd::Zeros(m_ui32SamplesPerChannel2);
				l_vecXdPhase1 = RowVectorXd::Zeros(m_ui32SamplesPerChannel1);
				l_vecXdPhase2 = RowVectorXd::Zeros(m_ui32SamplesPerChannel2);

	    		for(uint32 i=0; i<m_ui32SamplesPerChannel1; i++)
	    		{
			    	l_vecXdChannelToCompare1 = m_pChannelToCompare->getBuffer()[i + channel*m_ui32SamplesPerChannel1];
			    	l_vecXdChannelToCompare2 = m_pChannelToCompare->getBuffer()[i + (channel+1)*m_ui32SamplesPerChannel2];
	    		}
