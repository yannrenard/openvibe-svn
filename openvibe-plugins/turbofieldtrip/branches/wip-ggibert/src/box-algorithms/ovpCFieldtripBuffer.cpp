...
boolean CBoxAlgorithmFieldtripBuffer::initialize(void)
{
	...
	TParameterHandler < IMatrix* > l_pMatrix (m_pStreamDecoder->getOutputParameter(OVP_GD_Algorithm_StreamedMatrixStreamDecoder_OutputParameterId_Matrix));
	l_pMatrix->getBuffer();
...
boolean CBoxAlgorithmFieldtripBuffer::uninitialize(void)
{
...
boolean CBoxAlgorithmFieldtripBuffer::processInput(uint32 ui32InputIndex)
{
	getBoxAlgorithmContext()->markAlgorithmAsReadyToProcess();
	return true;
}

boolean CBoxAlgorithmFieldtripBuffer::process(void)
{
	...
	for(uint32 i=0; i<l_rDynamicBoxContext.getInputChunkCount(0); i++)
	{
		...
		if(m_pStreamDecoder->isOutputTriggerActive(OVP_GD_Algorithm_StreamedMatrixStreamDecoder_OutputTriggerId_ReceivedBuffer))
		{
			...
			{
				//~ if(*l_pBuffer < m_f64MinFieldtripBufferValue && (m_ui64FieldtripBufferMethod==OVP_TypeId_FieldtripBufferMethod_Min || m_ui64FieldtripBufferMethod==OVP_TypeId_FieldtripBufferMethod_MinMax)) *l_pBuffer=m_f64MinFieldtripBufferValue;
				//~ if(*l_pBuffer > m_f64MaxFieldtripBufferValue && (m_ui64FieldtripBufferMethod==OVP_TypeId_FieldtripBufferMethod_Max || m_ui64FieldtripBufferMethod==OVP_TypeId_FieldtripBufferMethod_MinMax)) *l_pBuffer=m_f64MaxFieldtripBufferValue;
			}
