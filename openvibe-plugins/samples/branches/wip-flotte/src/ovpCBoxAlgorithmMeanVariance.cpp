...
boolean CBoxAlgorithmMeanVariance::initialize(void)
{
	...
	m_ui64ActivityStartTime=(1LL<<32)*::atof(l_sSettingValue.toASCIIString());
	getStaticBoxContext().getSettingValue(1, l_sSettingValue);
	m_ui64ActivityEndTime=(1LL<<32)*::atof(l_sSettingValue.toASCIIString());
			...
			if(m_ui64StartTime >= m_ui64ActivityStartTime)
			{
				...
				else
				{
					if(!m_bFinished)
					{
						...
						l_pFile=fopen("../../laval-2008/records/current/cfg-dsp.xml", "wt");
						...
						l_pFile=fopen("../../laval-2008/records/current/cfg-minmax.xml", "wt");
						fprintf(l_pFile, "<OpenViBE-SettingsOverride>\n");
