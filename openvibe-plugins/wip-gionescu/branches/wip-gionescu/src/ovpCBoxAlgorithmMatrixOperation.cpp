...
CBoxAlgorithmMatrixOperation::CBoxAlgorithmMatrixOperation(void)
	: m_pAlgorithmMatrixOperation(0)
{
}

/*
uint64 CBoxAlgorithmMatrixOperation::getClockFrequency(void)
{
	return 0; // the box clock frequency
}
*/

boolean CBoxAlgorithmMatrixOperation::initialize(void)
{
	...
	if(!CMatrixOperation::parse(l_rStaticBoxContext.getInputCount(), m_grammar.toASCIIString()))
    {   std::cout << "GELU ERROR >>> " << __FILE__ << "::" << __LINE__ << " CBoxAlgorithmMatrixOperation::initialize" << std::endl;
		return false;
    }

	// Creates algorithms
	m_pAlgorithmMatrixOperation   = &getAlgorithmManager().getAlgorithm(getAlgorithmManager().createAlgorithm(OVP_ClassId_Algorithm_MatrixOperation));
	m_pAlgorithmMatrixOperation->initialize();
    ...
    TParameterHandler < IMatrix* > op_pMatrixToEncode(m_pAlgorithmMatrixOperation->getOutputParameter(OVP_Algorithm_MatrixOperation_OutputParameterId_Result));
...
boolean CBoxAlgorithmMatrixOperation::uninitialize(void)
{
    ...
    m_pAlgorithmMatrixOperation->uninitialize();
    	
    // release section
    getAlgorithmManager().releaseAlgorithm(*m_pAlgorithmMatrixOperation);
	
	return true;
}

/*
boolean CBoxAlgorithmMatrixOperation::processEvent(IMessageEvent& rMessageEvent)
{
	// ...
...
/*
boolean CBoxAlgorithmMatrixOperation::processSignal(IMessageSignal& rMessageSignal)
{
	// ...
...
/*
boolean CBoxAlgorithmMatrixOperation::processClock(IMessageClock& rMessageClock)
{
	// ...
...
boolean CBoxAlgorithmMatrixOperation::processInput(uint32 ui32InputIndex)
{
	IBox&	l_rStaticBoxContext     = this->getStaticBoxContext();
...
boolean CBoxAlgorithmMatrixOperation::process(void)
{
//	IBox& l_rStaticBoxContext=this->getStaticBoxContext();
	IBoxIO& l_rDynamicBoxContext=this->getDynamicBoxContext();
    ...
    {   std::cout << "GELU INFO >>> IsValid()" << std::endl;
        ...
        {   std::cout << "GELU ERROR >>> " << __FILE__ << "::" << __LINE__ << " CBoxAlgorithmMatrixOperation::process::IsValid " << std::endl;
    ...
    {   TParameterHandler < void* > ip_pMatrixes(m_pAlgorithmMatrixOperation->getInputParameter(OVP_Algorithm_MatrixOperation_InputParameterId_Inputs));
		TParameterHandler < OpenViBE::CString* > ip_pGrammar(m_pAlgorithmMatrixOperation->getInputParameter(OVP_Algorithm_MatrixOperation_InputParameterId_Grammar));

        ip_pMatrixes    = &m_matrixes;
        ip_pGrammar     = &m_grammar;
        
        m_pAlgorithmMatrixOperation->process(OVP_Algorithm_MatrixOperation_InputTriggerId_Evaluate);
        
        if(m_pAlgorithmMatrixOperation->isOutputTriggerActive(OVP_Algorithm_MatrixOperation_OutputTriggerId_Success))
...
OpenViBE::boolean CBoxAlgorithmMatrixOperation::IsValid(const std::vector<OpenViBE::IMatrix*>& matrixes,
    const std::vector<OpenViBE::uint64>& chunkStartTime, const std::vector<OpenViBE::uint64>& chunkEndTime)
{
	IBoxIO& l_rDynamicBoxContext=this->getDynamicBoxContext();
    
    if(matrixes.size() == 0)
    {   std::cout << "GELU ERROR >>> " << __FILE__ << "::" << __LINE__ << " CBoxAlgorithmMatrixOperation::IsValid" << std::endl;
		return false;
    }
    ...
    TParameterHandler < IMatrix* > op_iMatrix(m_pAlgorithmMatrixOperation->getOutputParameter(OVP_Algorithm_MatrixOperation_OutputParameterId_Result));
