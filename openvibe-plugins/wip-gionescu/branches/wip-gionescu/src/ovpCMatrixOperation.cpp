...
CMatrixOperation::CMatrixOperation(void)
{
}

bool CMatrixOperation::parse(const int nbEntries, const char* formula, const bool debug /*= false*/)
{
	bool	ret		= true;
    ...
    if(!ret)
    {   std::cout << "GELU ERROR >>> " << __FILE__ << "::" << __LINE__ << " CMatrixOperation::parse" << std::endl;
		return false;
    }
...
bool  CMatrixOperation::evaluate(OpenViBE::IMatrix& rResult, const std::vector<OpenViBE::IMatrix*>& rImputs, const OpenViBE::CString& rFormula)
{
    ...
    if(!ret)
    {   std::cout << "GELU ERROR >>> " << __FILE__ << "::" << __LINE__ << " CMatrixOperation::evaluate" << std::endl;
		return false;
    }
...
void CMatrixOperation::debug(const std::vector<OpenViBE::IMatrix*>& rImputs, const OpenViBE::IMatrix& rResult)
{
	for(std::vector<OpenViBE::IMatrix*>::const_iterator it=rImputs.begin(); it != rImputs.end(); it++)
