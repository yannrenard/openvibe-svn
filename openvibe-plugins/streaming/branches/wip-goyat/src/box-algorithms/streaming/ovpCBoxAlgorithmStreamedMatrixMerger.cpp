			...
			if(l_ui32DecoderType==DecoderType_Header)
			{
				...
				if(l_vui32dimensionCount.empty())
				{
					...
					if(m_ui32MergingDimension>=l_vui32dimensionCount.size())
					  {
						this->getLogManager() << LogLevel_Error << "Merging dimensions out of range : got " << m_ui32MergingDimension << "th dimension - wanted " << l_vui32dimensionCount.size() << " dimensions maximum.\n";
						return false;
					  }
				}
				else if(l_vui32dimensionCount.size()!=op_pDecodedMatrix1->getDimensionCount())
				{
					this->getLogManager() << LogLevel_Error << "Dimensions mismatch : wanted " << l_vui32dimensionCount.size() << " dimensions - got " << op_pDecodedMatrix1->getDimensionCount() << " dimensions.\n";
					return false;
				}
				else
				{
					for(uint32 h=0; h<l_vui32dimensionCount.size(); h++)
					  {
					    ...
					    if(l_vui32dimensionCount.at(h)!=op_pDecodedMatrix1->getDimensionSize(h))
						  {
						  	this->getLogManager() << LogLevel_Error << "Dimension size "<<h<<" mismatch : wanted " << l_vui32dimensionCount.at(h) << " - got " << op_pDecodedMatrix1->getDimensionSize(h) << ".\n";
							return false;
						  }
					  }
				}
