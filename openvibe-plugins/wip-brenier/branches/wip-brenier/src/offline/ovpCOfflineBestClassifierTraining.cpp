...
#include "ovpCOfflineBestClassifierTraining.h"
...
COfflineBestClassifierTraining::COfflineBestClassifierTraining(void)
	:m_oSignalReaderCallbackProxy(*this
		,&COfflineBestClassifierTraining::setChannelCount
		,&COfflineBestClassifierTraining::setChannelName
		,&COfflineBestClassifierTraining::setSampleCountPerBuffer
		,&COfflineBestClassifierTraining::setSamplingRate
		,&COfflineBestClassifierTraining::setSampleBuffer)
	,m_oStimulationReaderCallbackProxy(*this
		,&COfflineBestClassifierTraining::setStimulationCount
		,&COfflineBestClassifierTraining::setStimulation)
...
OpenViBE::boolean COfflineBestClassifierTraining::initialize()
{
...
void COfflineBestClassifierTraining::release(void)
{
	delete this;
}

boolean COfflineBestClassifierTraining::processInput(uint32 ui32InputIndex)
{
	getBoxAlgorithmContext()->markAlgorithmAsReadyToProcess();
	return true;
}

boolean COfflineBestClassifierTraining::process(void)
{
...
CIdentifier COfflineBestClassifierTraining::getStimulationIdentifierTrialStart(void)
{
	return m_oTrialStartIdentifier;
	//return 0x300;
}

CIdentifier COfflineBestClassifierTraining::getStimulationIdentifierTrialEnd(void)
{
	return m_oTrialEndIdentifier;
	//return 0x3FE;
}

CIdentifier COfflineBestClassifierTraining::getStimulationIdentifierTrialLabelRangeStart(void)
{
	return m_oLabelStartIdentifier;
	//return 0x00;
}

CIdentifier COfflineBestClassifierTraining::getStimulationIdentifierTrialLabelRangeEnd(void)
{
	return m_oLabelEndIdentifier;
	//return 0x01;
}

CIdentifier COfflineBestClassifierTraining::getStimulationIdentifierTrain(void)
{
	return m_oTrainIdentifier;
	//return 0x3FF;
}

boolean COfflineBestClassifierTraining::train(ISignalTrialSet& rTrialSet)
{
	...
	CLDAClassifier l_oBestClassifier;
	...
	for(float64 i=m_f64FeedbackStart ; i<=m_f64FeedbackEnd-m_f64WindowWidth ; i+=m_f64WindowStep)
	{
		...
		getBoxAlgorithmContext()->getPlayerContext()->getLogManager() << LogLevel_Trace << "Best classifier score : " << (float64)l_oBestClassifier.getAccuracy() << "% - Current classifier score : " << (float64)l_oCurrentClassifier.getAccuracy() << "%\n";

		//if the current classifier is the best one found up to now, it becomes the best classifier
		if(l_oCurrentClassifier.getAccuracy() > l_oBestClassifier.getAccuracy() )
		{
			l_oBestClassifier = l_oCurrentClassifier;
		}
	}
#else
...

	{
		...
		getBoxAlgorithmContext()->getPlayerContext()->getLogManager() << LogLevel_Trace << "kfold classifier score : " << (float64)l_oCurrentClassifier.getAccuracy() << "%\n";
	...
	FeatVecSet l_oFeatureVectorSet = m_oBandPower.createDataSet(l_oTrialSet);
	l_oBestClassifier.train(l_oFeatureVectorSet);
	l_oBestClassifier.test(l_oFeatureVectorSet);
	getBoxAlgorithmContext()->getPlayerContext()->getLogManager() << LogLevel_Trace << "Test on training set, best classifier score : " << l_oBestClassifier.getAccuracy() << "%\n";

	getBoxAlgorithmContext()->getPlayerContext()->getLogManager() << LogLevel_Trace << "Done! Saving parameters...\n";
	l_oBestClassifier.saveParams((const char*)m_oOutputConfiguration);
	...
	vector<double> l_oCoefficients = l_oBestClassifier.getCoefficients();
