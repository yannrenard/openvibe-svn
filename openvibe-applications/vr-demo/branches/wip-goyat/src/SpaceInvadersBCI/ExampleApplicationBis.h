...
public:
    ...
    ExampleApplication()
    {
        ...
        mConfigPath = mResourcePath;
#else
		mResourcePath = std::string(getenv("OGRE_HOME"))+std::string("/bin/debug/plugins_d.cfg");
        mConfigPath = mResourcePath;
#endif
    ...
    {
		std::cout<<"exempleAppBis go"<<std::endl;
        if (!setup())
            return;
		std::cout<<"exempleAppBis  setup ok"<<std::endl;
		mRoot->getRenderSystem()->setConfigOption("VSync", "True");
		std::cout<<"exempleAppBis VSync ok"<<std::endl;
        mRoot->startRendering();
		std::cout<<"exempleAppBis rendering ok"<<std::endl;
