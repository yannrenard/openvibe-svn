	...
	IDriverCallback& rCallback)
{
	...
	if (m_structRDA_MessageHeader.nType !=1)
	{
	...
	m_pStructRDA_MessageStart = (RDA_MessageStart*) (m_pcharStructRDA_MessageBuffer+sizeof(m_structRDA_MessageHeader));

	cout << "> Header received" << std::endl;
	cout << "> Waiting for first block of data to know number of samples per block... " << std::endl;
	...
	free(m_pcharStructRDA_MessageBuffer);

	// Read first buffer of data to get number of samples per block
	readBrainampHeader();
	...
	if (m_structRDA_MessageHeader.nType == 3)
	{
		cout << "Message received is a STOP!" << std::endl;
		m_bStarted = false;
		return false;
	}
	if (m_structRDA_MessageHeader.nType !=4)
	{
		cout << "Message received is %d ." << std::endl;
	}
	
	// Put the data into MessageData32 structure
	m_pStructRDA_MessageData32 = (RDA_MessageData32*)(m_pcharStructRDA_MessageBuffer+sizeof(m_structRDA_MessageHeader));
	
	cout << "> Done. Number of sample per block = " << m_pStructRDA_MessageData32->nPoints << std::endl;
	
	if(!rCallback.forceSampleCountPerSentBuffer(m_pStructRDA_MessageData32->nPoints))
		return false;
//	if(ui32RequestedSampleCountPerSentBlock==0)
//	{
		// TODO Please choose a nice sample count per sent block
		//
		// if(!rCallback.forceSampleCountPerSentBuffer(/*whatever*/))
		//
...
//	}
	...
	m_ui32SampleCountPerSentBlock = m_pStructRDA_MessageData32->nPoints;
	...
	else
	{
		...
		if (m_ui32SampleCountPerSentBlock > (uint32)m_pStructRDA_MessageData32->nPoints)
		{
			...
			for (uint32 i = l_ui32NumberOfMarkersToSend; i < m_ui32NumberOfMarkers; i++)
			{
				...
				l_vStimulationDate[i-l_ui32NumberOfMarkersToSend] = (((uint64)(m_vStimulationSample[i] - m_ui32SampleCountPerSentBlock)) << 32 ) / m_oHeader.getSamplingFrequency();
			}
...
boolean CDriverBrainAmpScalpEEG::uninitialize(void)
{
	...
	if (m_pcharStructRDA_MessageBuffer!=NULL) m_pcharStructRDA_MessageBuffer=NULL;
...
// Read BRAINAMP Header
boolean CDriverBrainAmpScalpEEG::readBrainampHeader(void)
{
	...
	char *l_pcharStructRDA_MessageHeader = (char *)&m_structRDA_MessageHeader;
	char *l_pcharStructRDA_MessageBuffer;

	uint32 l_ui32Received = 0;
	uint32 l_ui32ReqLength = 0;
	uint32 l_ui32Result = 0;
	uint32 l_ui32Datasize = sizeof(m_structRDA_MessageHeader);

	// Receive Header
	while(l_ui32Received < l_ui32Datasize)
	{
		l_ui32ReqLength = l_ui32Datasize -  l_ui32Received;
		l_ui32Result = m_pConnectionClient->receiveBuffer((char*)l_pcharStructRDA_MessageHeader, l_ui32ReqLength);

		l_ui32Received += l_ui32Result;
		l_pcharStructRDA_MessageHeader += l_ui32Result;
	}

	// Check for correct header GUID.
	if (!COMPARE_GUID(m_structRDA_MessageHeader.guid, GUID_RDAHeader))
	{
	...
	if (!(m_pcharStructRDA_MessageBuffer = (char *)malloc(m_structRDA_MessageHeader.nSize)))
	{
	...
	{
		memcpy(m_pcharStructRDA_MessageBuffer, &m_structRDA_MessageHeader, sizeof(m_structRDA_MessageHeader));
		l_pcharStructRDA_MessageBuffer = m_pcharStructRDA_MessageBuffer+sizeof(m_structRDA_MessageHeader);
		l_ui32Received=0;
		l_ui32ReqLength = 0;
		l_ui32Result = 0;
		l_ui32Datasize = m_structRDA_MessageHeader.nSize - sizeof(m_structRDA_MessageHeader);
		while(l_ui32Received < l_ui32Datasize)
		{
			l_ui32ReqLength = l_ui32Datasize -  l_ui32Received;
			l_ui32Result = m_pConnectionClient->receiveBuffer((char*)l_pcharStructRDA_MessageBuffer, l_ui32ReqLength);

			l_ui32Received += l_ui32Result;
			l_pcharStructRDA_MessageBuffer += l_ui32Result;
		}
	}
