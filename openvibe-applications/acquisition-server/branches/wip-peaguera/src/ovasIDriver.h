...
namespace OpenViBEAcquisitionServer
{
	...
	public:
		...
		/**
		 * \brief Forces a new sample count per sent buffer
		 * \param ui32RequestedSampleCountPerSentBlock [in] : the new sample count per sent
		 *        buffer to consider in the acquisition server
		 * \return \e true in case of success.
		 * \return \e false in case of error.
		 *
		 * This function allows your driver to force the sample count per
		 * sent buffer if the value that is sent at initialization stage
		 * does not fit your needs. This function should only be called in
		 * the \c initialize function of your driver. If called outside of
		 * this context, it will always return \e false.
		 *
		 * If this function returns \e false, the acquisition server
		 * expects you use the old sample count per buffer. If you really
		 * can't manage this, then your \c initialize function should fail
		 * and return \e false too !
		 */
		virtual OpenViBE::boolean forceSampleCountPerSentBuffer(
			const OpenViBE::uint32 ui32NewSampleCountPerSentBuffer)=0;
		...
		/**
		 * \brief Gives new sample buffer
		 * \param pSample [in] : a buffer containing some samples
		 *
		 * This is used by the acquisition server to be notified when the
		 * driver can send new samples. Using this callback does not need
		 * to setup a complete buffer. The acquisition server will take care
		 * of grouping the samples altogether in a complete buffer for you.
		 * This function is called by the driver during the \e IDriver::loop
		 * and should give an array of \c nSamplesPerChannel x \c nChannel
		 * organised by channel first.
		 *
		 ...
		 * pSample[1] is channel 0 sample 1
		 * ...
		 * pSample[ui32SampleCount-1] is channel 0 sample ui32SampleCount-1
		 * pSample[ui32SampleCount  ] is channel 1 sample 0
		 * pSample[ui32SampleCount+1] is channel 1 sample 1
		 * ...
		 * pSample[i*ui32SampleCount+j] is channel i sample j
		 * \endcode
		 *
		 * \sa IDriver::loop
		 */
		 ...
		 *          first sample date of the last \c setSamples call.
		 *
		 * \sa IDriver::loop
		 */
	...
	public:
		 ...
		 * \param ui32RequestedSampleCountPerSentBlock [in] : the number of samples to
		 ...
		 *
		 * The \c ui32RequestedSampleCountPerSentBlock value should be considered when using
		 * the 1 parameter version of IDriverCallback::setSamples function. If your
		 * driver wants to override this value, it will have to call the two parameters
		 * version of the IDriverCallback::setSamples function to give the acquisition
		 * server the information of how many samples are sent at each loop step.
		 *
		 * \sa uninitialize
		 * \sa getHeader
		 * \sa IHeader
