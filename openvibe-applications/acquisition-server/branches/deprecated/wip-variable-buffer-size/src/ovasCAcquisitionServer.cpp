...
void CAcquisitionServer::idleCB(void)
{
	...
	// Sends data to connected client(s) when enough samples have been received
	if(m_bStarted && m_ui32PendingSampleCount>=m_ui32SampleCountPerSentBlock)
	{
		m_rKernelContext.getLogManager() << LogLevel_Debug << "Creating buffer\n";

		// Prepares signal matrix
		// It is the same for all clients so it can be prepared out of the client loop
		...
		uint32 i, j;
		float64* l_pMatrixBuffer=ip_pMatrix->getBuffer();
		float64* l_pPendingMatrixBuffer=m_oPendingMatrix.getBuffer();
		for(i=0; i<m_ui32ChannelCount; i++)
		{
			System::Memory::copy(l_pMatrixBuffer, l_pPendingMatrixBuffer, m_ui32SampleCountPerSentBlock*sizeof(float64));
			for(j=0; j<m_ui32PendingSampleCount-m_ui32SampleCountPerSentBlock; j++)
			{
				l_pPendingMatrixBuffer[j]=l_pPendingMatrixBuffer[j+m_ui32SampleCountPerSentBlock];
			}
			l_pMatrixBuffer+=m_ui32SampleCountPerSentBlock;
			l_pPendingMatrixBuffer+=m_ui32MaximumPendingSampleCount;
		}
		...
		while(itConnection!=m_vConnection.end())
		{
			...
			{
				// When the connection is no more valid, remove it
			...
			{
				// Prepares stimulation matrix
				// It can be different depending on the client because of the stimulation dating
				...
				OpenViBEToolkit::Tools::StimulationSet::copy(*ip_pStimulationSet, m_oPendingStimulationSet, l_ui64TimeOffset);
		...
		// Updates sample count values
		m_ui32PendingSampleCount-=m_ui32SampleCountPerSentBlock;
...
#else
	System::Time::zsleep((1LL<<32)/1024);
#endif
}
...
void CAcquisitionServer::buttonConnectToggledCB(::GtkToggleButton* pButton)
{
	...
	::GtkWidget* l_pStatusBox=glade_xml_get_widget(m_pGladeInterface, "vbox_status");

	if(gtk_toggle_button_get_active(pButton))
	{
		m_ui64SentSampleCount=0;
		m_ui32PendingSampleCount=0;
		...
		{
			if(m_ui32SampleCountPerSentBlock!=0)
			{
				...
				{
					// Everything was OK, now go !
					m_ui32MaximumPendingSampleCount=m_ui32SampleCountPerSentBlock*16;

					gtk_widget_show(l_pStatusBox);
					...
					ip_ui64BuferDuration=(((uint64)m_ui32SampleCountPerSentBlock)<<32)/m_ui32SamplingFrequency;
					...
					m_oPendingMatrix.setDimensionCount(2);
					m_oPendingMatrix.setDimensionSize(0, m_ui32ChannelCount);
					m_oPendingMatrix.setDimensionSize(1, m_ui32MaximumPendingSampleCount);
					m_ui32PendingSampleCount=0;
				...
				else
				{
					...
					gtk_label_set_label(GTK_LABEL(glade_xml_get_widget(m_pGladeInterface, "label_status")), "Scoket connection failed !");
				}
			}
			else
			{
				// Uninitializes driver
				gtk_toggle_button_set_active(pButton, false);
				gtk_label_set_label(GTK_LABEL(glade_xml_get_widget(m_pGladeInterface, "label_status")), "Sample count per sent block initialization failed !");
			}
		}
		else
		{
			// Uninitializes driver
			gtk_toggle_button_set_active(pButton, false);
			gtk_label_set_label(GTK_LABEL(glade_xml_get_widget(m_pGladeInterface, "label_status")), "Driver initialization failed !");
		}
	}
	else
	{
...
boolean CAcquisitionServer::forceSampleCountPerSentBuffer(const uint32 ui32NewSampleCountPerSentBuffer)
{
	if(m_bInitialized)
	{
		return false;
	...
	if(ui32NewSampleCountPerSentBuffer==0)
	{
		return false;
	}

	m_ui32SampleCountPerSentBlock=ui32NewSampleCountPerSentBuffer;

	m_rKernelContext.getLogManager() << LogLevel_Warning << "The sample count per sent buffer has been forced to " << m_ui32SampleCountPerSentBlock << " by the driver.\n";
...
void CAcquisitionServer::setSamples(const float32* pSample, uint32 ui32SampleCount)
{
	if(m_bStarted)
	{
		uint32 l_ui32SampleToKeepCount=min(ui32SampleCount, m_oPendingMatrix.getDimensionSize(1)-m_ui32PendingSampleCount);

		if(l_ui32SampleToKeepCount != ui32SampleCount)
		{
			// some data will be dropped !
			m_rKernelContext.getLogManager() << LogLevel_ImportantWarning << "dropped data\n";
		}

		const float32* l_pSampleBuffer=pSample;
		float64* l_pPendingMatrixBuffer=m_oPendingMatrix.getBuffer()+m_ui32PendingSampleCount;
		for(uint32 i=0; i<m_ui32ChannelCount; i++)
		{
			for(uint32 j=0; j<l_ui32SampleToKeepCount; j++)
			{
				l_pPendingMatrixBuffer[j]=l_pSampleBuffer[j];
			}
			l_pPendingMatrixBuffer+=m_ui32MaximumPendingSampleCount;
			l_pSampleBuffer+=ui32SampleCount;
		}

		m_ui64LocalTimeOffset=(((uint64)m_ui32PendingSampleCount)<<32)/m_ui32SamplingFrequency;
		m_ui32PendingSampleCount+=l_ui32SampleToKeepCount;

#if 1
// m_rKernelContext.getLogManager() << LogLevel_ImportantWarning << "Pending : " << m_ui32PendingSampleCount << "\n";
		// Updates buffer filling status
		gdouble l_fFraction=m_ui32PendingSampleCount*1./m_ui32SampleCountPerSentBlock;
		gtk_widget_show(glade_xml_get_widget(m_pGladeInterface, "progressbar_buffer"));
		gtk_progress_bar_set_fraction(
			GTK_PROGRESS_BAR(glade_xml_get_widget(m_pGladeInterface, "progressbar_buffer")),
			min(l_fFraction, 1.));
#endif
	}
	else
	...
	if(m_bStarted)
	{
		for(uint64 i=0; i<rStimulationSet.getStimulationCount(); i++)
		{
			m_oPendingStimulationSet.appendStimulation(
				rStimulationSet.getStimulationIdentifier(i),
				rStimulationSet.getStimulationDate(i)+m_ui64LocalTimeOffset,
				rStimulationSet.getStimulationDuration(i));
		}
	}
	else
	{
