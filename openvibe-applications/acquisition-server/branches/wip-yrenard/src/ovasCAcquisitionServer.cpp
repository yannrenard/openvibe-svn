...
#include "mitsar-eeg202/ovasCDriverMitsarEEG202.h"
	...
	,m_pDriver(NULL)
{
	...
	m_vDriver.push_back(new CDriverMitsarEEG202());
#endif
...
void CAcquisitionServer::buttonConnectToggledCB(::GtkToggleButton* pButton)
{
	...
	if(gtk_toggle_button_get_active(pButton))
	{
		...
		// Initializes driver
		l_bError|=!m_pDriver->initialize(m_ui32SampleCountPerSentBlock, *this);
		...
		l_bError|=!m_pConnectionServer->listen(l_ui32ConnectionPort);
		...
		{
			m_rKernelContext.getLogManager() << LogLevel_ImportantWarning << "Driver initialization failed.\n";
...
void CAcquisitionServer::buttonStartPressedCB(::GtkButton* pButton)
{
	...
	if(!m_pDriver->start())
	{
		m_rKernelContext.getLogManager() << LogLevel_ImportantWarning << "Driver startup process failed.\n";
		return;
	}
