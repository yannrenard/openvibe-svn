...
///Soft Eye 1 - 500 Hz
///////////////////////////////////////////////////////////////////////////////////////////////////////////
using namespace Standard_SoftEye_500Hz;
...
//""""""""""""""""""""""""""""""
namespace global_SoftEye_500Hz
{

// Buffer de reception
eyelink_type500 reception_buffer_SoftEye_500Hz_struct[NB_SAMPLES];

// Buffer d'initialisation
synchro_type500 Struct_init;
...
using namespace global_SoftEye_500Hz;
...
CDriverSoftEye_500Hz::CDriverSoftEye_500Hz(IDriverContext& rDriverContext)
...
void CDriverSoftEye_500Hz::release(void)
{
	delete this;
}

const char* CDriverSoftEye_500Hz::getName(void)
{
	return "Eyelink 500 Hz (through SoftEye - IP 100.1.1.2 :: 700)";
}
...
//                                                                   //
	...
	// OS : Windows //////////////////////////////
    //#if defined (WIN32)
        WSADATA WSAData_SoftEye_500Hz;
        int l_flag_error_SoftEye_500Hz=WSAStartup(MAKEWORD(2,2), &WSAData_SoftEye_500Hz);
    //#else
	//	int l_flag_error_SoftEye_500Hz=0;
	//#endif
    //////////////////////////////////////////////
	/* Socket et contexte d'adressage du serveur */
    SOCKADDR_IN l_sin_SoftEye_500Hz;
    SOCKET l_sock_SoftEye_500Hz;
...
#include <iostream>
boolean CDriverSoftEye_500Hz::initialize(
	...
	IDriverCallback& rCallback)
{
	...
	//m_pSample=new openvibe_type500[NB_SAMPLES];
	//std::cout << "Sizeof struct openvibe_type500 : " << sizeof(openvibe_type500)/sizeof(float32)	<< std::endl;
	//system("pause");
	//m_pSample=(openvibe_type500)

	m_pCallback=&rCallback;
	m_bInitialized=true;
	m_bTCP_IP_link_Opened=false; // True if the TCP IP connection between softEye and the driver is ready to send data.
								 // This connection is ready when the eyelink calibration has been completed. (-Exit setup- in the eyelink interface) 
	m_ui32cptWaitTCP_IP=0;		 // for TCP IP waiting consol animation.
	...
	m_rDriverContext.getLogManager() << LogLevel_Error << "INIT : Not OS-Win32\n";
...
boolean CDriverSoftEye_500Hz::start(void)
{
#if defined OVAS_OS_Windows
	...
	/*
	if(!m_bTCP_IP_link_Opened)
	{
		m_rDriverContext.getLogManager() << LogLevel_Warning << "SoftEye<->OpenVibe Driver TPC-IP link not opened.\n";
		return false;
	}
	*/
        ...
        l_sock_SoftEye_500Hz = socket(AF_INET, SOCK_STREAM, 0);

		/* Configuration de la connexion */
        //l_sin.sin_addr.s_addr = inet_addr("127.0.0.1");
		l_sin_SoftEye_500Hz.sin_addr.s_addr = inet_addr(IP);
        l_sin_SoftEye_500Hz.sin_family = AF_INET;
        l_sin_SoftEye_500Hz.sin_port = htons(PORT);

		/* Si connection OK */
        //printf("Searching Device...\n");
		m_rDriverContext.getLogManager() << LogLevel_Info << "Searching Device...\n";
		if(connect(l_sock_SoftEye_500Hz, (SOCKADDR*)&l_sin_SoftEye_500Hz, sizeof(l_sin_SoftEye_500Hz)) != SOCKET_ERROR)
        {
            printf("Connection à %s sur le port %d\n", inet_ntoa(l_sin_SoftEye_500Hz.sin_addr), htons(l_sin_SoftEye_500Hz.sin_port));
			...
			int32 oct=0;
			oct=recv(l_sock_SoftEye_500Hz, (char *)&Struct_init, sizeof(Struct_init), 0);
			

			if(oct==SOCKET_ERROR)
			{
				//printf("SOCKET_ERROR - fonction recv()\n");
				m_rDriverContext.getLogManager() << LogLevel_Error << "SOCKET_ERROR - fonction recv()\n";
		...
		{
			m_rDriverContext.getLogManager() << LogLevel_Error << "Device not found.\n";
...
void CDriverSoftEye_500Hz::dump(openvibe_type500* pSample,	eyelink_type500* pEyelink)
{
...
boolean CDriverSoftEye_500Hz::loop(void)
{
#if defined OVAS_OS_Windows
	...
	oct=recv(l_sock_SoftEye_500Hz, (char *)&reception_buffer_SoftEye_500Hz_struct[0], sizeof(reception_buffer_SoftEye_500Hz_struct), 0);
	//printf("recv : %c %c %c %c ",reception_buffer_SoftEye_500Hz[0],reception_buffer_SoftEye_500Hz[1],reception_buffer_SoftEye_500Hz[2],reception_buffer_SoftEye_500Hz[3]);
	//printf("Data received : %d octets.\n",oct);


	/*
	// !!! Nouvelle version d'OpenVibe. L'appel systématique de la fonction loop, avant même que la fonction start soit appelée, est problématique car la connection TCP/IP n'est alors
	// toujours pas effective. Le test a donc été commenté. Le signe ">" est affiché sur la console pour chaque appel de loop tant que la connection n'est pas établie.
	// La procédure de connection n'a pas été déplacée de la fonction start à la fonction init afin de ne pas perdre la synchronisation, relativement au protocole que nous avions fixé
	// précédement. En revanche, le test qu'a implémenté Yann concernant la vérification de la fréquence d'échantillonage risque d'être biaisé (à voir).
	...
	*/
	if(oct==SOCKET_ERROR)
	{
		//Attente de la connection TCP/IP SoftEye<->Driver OpenVibe;
		m_ui32cptWaitTCP_IP++;
		printf("\xdc");
		
		if(m_ui32cptWaitTCP_IP==30)
		{
			printf("\x0d                              \x0d");
			m_ui32cptWaitTCP_IP=0;
		}
	...
	int32 oct_cut;
	while(oct<sizeof(reception_buffer_SoftEye_500Hz_struct))
	{
		//printf(">");
		oct_cut=oct;
		oct=recv(l_sock_SoftEye_500Hz, (char *)&reception_buffer_SoftEye_500Hz_struct[oct_cut], sizeof(reception_buffer_SoftEye_500Hz_struct)-oct_cut, 0);
	...
	if(oct==sizeof(reception_buffer_SoftEye_500Hz_struct))
	{
		
		//printf("\nPrepare data...\n");
		
			//float32* pSample=m_pSample;
			eyelink_type500* pEyelink=reception_buffer_SoftEye_500Hz_struct;
		...
		///**************************************** correction Drift...................................//
		if(m_ui32DriftCorrectionState) // m_ui32DriftCorrectionState is the Drift correction activation flag : should be activated via the graphical user interface. 
		{
	...
	{
		m_rDriverContext.getLogManager() << LogLevel_Error << "ERROR packet size.\n";
...
boolean CDriverSoftEye_500Hz::stop(void)
{
#if defined OVAS_OS_Windows

	if(!m_bInitialized)
	{
		m_rDriverContext.getLogManager() << LogLevel_Error << "Not Initialized.\n";
	...
	if(!m_bStarted)
	{
		m_rDriverContext.getLogManager() << LogLevel_Error << "Not Started.\n";
...
/*
	...
	return !m_bStarted;
*/
			m_rDriverContext.getLogManager() << LogLevel_Info << "Shutdown...\n";
			//printf("Shutdown...\n");
			shutdown(l_sock_SoftEye_500Hz, 2);
			/* On ferme la socket */
			m_rDriverContext.getLogManager() << LogLevel_Info << "Close socket...\n";
            //printf("Close socket...\n");
			closesocket(l_sock_SoftEye_500Hz);
	        return true;	
...
boolean CDriverSoftEye_500Hz::uninitialize(void)
{
#if defined OVAS_OS_Windows

	if(!m_bInitialized)
	{
		m_rDriverContext.getLogManager() << LogLevel_Error << "ERROR not Initialized.\n";
		//printf("ERROR Init flag.\n");
		return false;
	}
	...
	if(m_bStarted)
	{
		m_rDriverContext.getLogManager() << LogLevel_Error << "ERROR not Stopped.\n";
		//printf("ERROR Start flag.\n");
		return false;
	}
	...
	m_rDriverContext.getLogManager() << LogLevel_Info << "Cleanup...\n";
	//printf("Cleanup...");
	WSACleanup();
	...
	m_rDriverContext.getLogManager() << LogLevel_Info << "SUCCESS.\n";
	//printf("SUCCESS.\n");
	return true;
...
boolean CDriverSoftEye_500Hz::isConfigurable(void)
{
#if defined OVAS_OS_Windows

	// If the new interface .ui with the DriftCorrection check box exists, return true.
	 return true;
	// else
	//return false;
...
boolean CDriverSoftEye_500Hz::configure(void)
{
#if defined OVAS_OS_Windows
		
	// If the new interface .ui with the DriftCorrection check box exists :
	//CConfigurationSoftEye_500Hz m_oConfiguration("../share/openvibe-applications/acquisition-server/interface_SoftEye500Hz.ui", m_ui32DriftCorrectionState);
	CConfigurationSoftEye_500Hz m_oConfiguration("../share/openvibe-applications/acquisition-server/interface-SoftEye500Hz.ui", m_ui32DriftCorrectionState);
	// else
	//CConfigurationSoftEye_500Hz m_oConfiguration("../share/openvibe-applications/acquisition-server/interface_SoftEye500Hz.ui");
