	...
	,m_bCorrectNonIntegerSR(true)
{
    ...
    CString sEnvVar = "GET_CPU_TIME";
    CString sValue = "";
    /*if ( GetEnvVar( sEnvVar, sValue ) )
    {
        if (sValue != CString("0"))
            m_bGetCpuTime = true;
    }
    ...
    if (m_bGetCpuTime)
    {
        sEnvVar = "GET_CPU_TIME_MEASURES_FOLDER";
        sValue = "";
        if ( GetEnvVar( sEnvVar, sValue ) )
        {
            m_sMeasureFolder = sValue;
        }
        else
        {
            PutEnvVar( sEnvVar, m_sMeasureFolder );
        }

        sEnvVar = "GET_CPU_TIME_DETECTION_THRESHOLD";
        sValue = "";
        if ( GetEnvVar( sEnvVar, sValue ) )
        {
            m_f64DetectionThreshold = 1000000.0*atof(sValue.toASCIIString());
        }
        else
        {
            sValue = "0.0";
            PutEnvVar( sEnvVar, sValue );
        }
        
        sEnvVar = "GET_CPU_TIME_DETECTION_HIGHER";
        sValue = "";
        if ( GetEnvVar( sEnvVar, sValue ) )
        {
            m_bDetectionHigher = (sValue == CString("0")? false : true);
        }
        else
        {
            sValue = "1";
        ...
        sEnvVar = "GET_CPU_TIME_DETECTION_CHANNEL";
        sValue = "";
        if ( GetEnvVar( sEnvVar, sValue ) )
        {
            m_ui32DetectionChannel = atoi(sValue.toASCIIString());
        }
        else
        {
            sValue = "0";
	...
	IDriverCallback& rCallback)
{
    ...
    if (m_bGetCpuTime)
    {
		std::string sFile = m_sMeasureFolder.toASCIIString();
        sFile += "cpu_time_driverFieldtrip_T3.txt";
        m_myfile = fopen(sFile.c_str(),"w");
        if (!m_myfile)
        {
            m_rDriverContext.getLogManager() << LogLevel_Warning << "Can't open cpu-time file " << sFile.c_str() << "\n";
            m_bGetCpuTime = false;
        }
        m_f64mesureLostTime = 0;
        m_ui32mesureNumber = 0;
