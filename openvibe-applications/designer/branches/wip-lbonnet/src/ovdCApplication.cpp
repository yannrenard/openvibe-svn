...
namespace
{
	...
	extern "C" G_MODULE_EXPORT void load_configuration_file_cb(::GtkButton* pButton, gpointer pUserData)
	{
		static_cast<CApplication*>(pUserData)->loadConfigurationFileCB();
	}
...
CApplication::CApplication(const IKernelContext& rKernelContext)
	...
	,m_pPreferencesBuilderInterface(NULL)
	,m_pMainWindow(NULL)
...
void CApplication::preferencesCB(void)
{
	...
	m_pPreferencesBuilderInterface=gtk_builder_new(); // glade_xml_new(OVD_GUI_File, "configuration_manager", NULL);
	gtk_builder_add_from_file(m_pPreferencesBuilderInterface, OVD_GUI_File, NULL);
	gtk_builder_connect_signals(m_pPreferencesBuilderInterface, NULL);
	g_signal_connect(gtk_builder_get_object(m_pPreferencesBuilderInterface, "configuration_manager-button_load"), "pressed",G_CALLBACK(load_configuration_file_cb), this);
	//::GtkBuilder* l_pBuilderInterface = m_pBuilderInterface;
	::GtkWidget* l_pConfigurationManager=GTK_WIDGET(gtk_builder_get_object(m_pPreferencesBuilderInterface, "configuration_manager"));
	
	refreshTokenTreeview(true);
	...
	g_object_unref(m_pPreferencesBuilderInterface);
	m_pPreferencesBuilderInterface = NULL;
}
...
void CApplication::addCommentCB(CInterfacedScenario* pScenario)
{
...
void CApplication::loadConfigurationFileCB()
{
	m_rKernelContext.getLogManager() << LogLevel_Trace << "Load config file CB\n";
	::GtkWidget* l_pWidgetDialogOpen=gtk_file_chooser_dialog_new(
		"Select an OpenViBE configuration file",
		NULL,
		GTK_FILE_CHOOSER_ACTION_OPEN,
		GTK_STOCK_CANCEL, GTK_RESPONSE_CANCEL,
	...
	if(gtk_dialog_run(GTK_DIALOG(l_pWidgetDialogOpen))==GTK_RESPONSE_ACCEPT)
	{
		char * l_sFile = gtk_file_chooser_get_filename(GTK_FILE_CHOOSER(l_pWidgetDialogOpen));
		m_rKernelContext.getConfigurationManager().addConfigurationFromFile(CString(l_sFile));
		this->refreshTokenTreeview(false);
	}
	gtk_widget_destroy(l_pWidgetDialogOpen);
}

void CApplication::refreshTokenTreeview(boolean bFillTreeview)
{
	enum
	{
		Resource_TokenName,
	...
	if(m_pPreferencesBuilderInterface == NULL)
	{
		m_rKernelContext.getLogManager() << LogLevel_Error << "No \"Preferences\" window opened. This should never happened when calling refreshTokenTreeview().\n";
	}
	
	::GtkWidget* l_pConfigurationManager=GTK_WIDGET(gtk_builder_get_object(m_pPreferencesBuilderInterface, "configuration_manager"));
	::GtkTreeView* l_pConfigurationManagerTreeView=GTK_TREE_VIEW(gtk_builder_get_object(m_pPreferencesBuilderInterface, "configuration_manager-treeview"));
	if(bFillTreeview)
	{
		// Prepares tree view
	...
	g_signal_emit_by_name(gtk_tree_view_get_column(l_pConfigurationManagerTreeView,0), "clicked");
